...
def fbx_data_bindpose_element(root, me_obj, me, scene_data, arm_obj=None, mat_world_arm=None, bones=[]):
    ...
    if scene_data.settings.stellar_blade_fix:
        skeleton_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "sb-json", skeleton_file + ".json")
        try:
            with open(skeleton_path, "r", encoding="utf-8") as file:
                skeleton_json = json.load(file)
            scene_data.settings.report({'WARNING'}, f"剑星骨骼翻转：正在使用骨骼文件 {skeleton_file}.json")
        except OSError as ex:
            scene_data.settings.report({'ERROR'}, f"剑星骨骼翻转：无法读取骨骼文件：{ex}")
    ...
    for bo_obj in bones:
        ...
        if skeleton_json is not None and bo_obj.name in skeleton_json[-1]["ReferenceSkeleton"]["FinalNameToIndexMap"]:
            _loc, _rot, scale = bomat.decompose()
            bone_json = skeleton_json[-1]["ReferenceSkeleton"]["FinalRefBonePose"][
                skeleton_json[-1]["ReferenceSkeleton"]["FinalNameToIndexMap"][bo_obj.name]
            ]
            scale_should = np.array([bone_json["Scale3D"]["X"], bone_json["Scale3D"]["Y"], bone_json["Scale3D"]["Z"]])
            scale_is = np.array([scale.x, scale.y, scale.z]) * (
                -1 if (bo_obj in bones_actually_flipped and bones_actually_flipped[bo_obj]) else 1
            )
            should_flip = np.dot(scale_should, scale_is) < 0
...
def save_single(operator, scene, depsgraph, filepath="",
    ...
    settings = FBXExportSettings(
        ...
        stellar_blade_fix, stellar_blade_skeleton
    )
